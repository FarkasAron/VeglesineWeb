...
interface Props {
  searchParams: { tema?: string; szint?: string; nehezseg?: string; ev?: string; nezet?: string };
}
...
export default async function FeladatokPage({ searchParams }: Props) {
  ...
  const titleParts = [topicLabel, examLabel, diffLabel, searchParams.ev].filter(Boolean);
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{problems.length} feladat</p>
        </div>
        <Suspense>
